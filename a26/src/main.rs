fn main() {
    // <1> Executable projects require a main() function
    let penguin_data = "\
  ...
  ";
    ...
    for (i, record) in records.enumerate() {
        if i == 0 || record.trim().len() == 0 {
            // <2> Skip header row and lines with only whitespace
            continue;
        }
        ...
        if cfg!(debug_assertions) {
            // <6> When debugging is enabled, include this code block. The exclamation mark (!) indicates a macro invocation.
